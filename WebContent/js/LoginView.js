...
//            var lblSpaceLegend = new sap.ui.commons.Label( {text: " ",width : '5px'});
            var vTxtSeaweb = new sap.m.Text({text: "Internal Sales Order"}).addStyleClass("fontWelcomeRed");
            ...
            sap.ui.commons.PasswordField.prototype.onkeyup = function(e) {
                var k = e.which || e.keyCode;
						...
						{	
							...
							oCurrent.LogonBtnPress(password);
						}
          ...
          layoutData: new sap.ui.layout.GridData({span: "L4 M3 S4"}),
                   tap: function() {
                       ...
                       //isAuthentic = oCurrent.authenticateUser();
//                       if(isAuthentic){
...
//                       }
            ...
            var vTxtChngPswd = new sap.m.Text({text: "To Change Password Click"}).addStyleClass("font13");
            ...
            var vLinkHereLink = new sap.ui.commons.Link({
                   ...
                   press:function(){
                	   ...
                	   var password = document.getElementById("idtxtPassword").value;
                    	  // oCurrent.ChangeBtnPress(password);
                    	  var oLoginView = new LoginView();
                	      oLoginView.changePassword(password);
                   }
            }).addStyleClass("font13 marginTop2");
            ...
            var idFlexLoginForm = "idFlexLoginFormLap";
            if(isMobile.any()){
            	idFlexLoginForm = "idFlexLoginForm";
            }
            ...
            return oFlexboxLoginForm;

	},
	passwordNotOk: function(){
		sap.m.MessageBox.alert("Please check the passwords and retry!");
	},
	
	callBackend: function(userId, password, newPassword, conPassword){
		
		oModel = new sap.ui.model.odata.ODataModel(serviceUrl, true, globaluser, globalpass);
		var vApptype = "GRNAP"; //'GRNAP' for GRN //'SALES' for Sales collect // 'VANSC' for Van sales // 'PRODR' for Production order// 'CUSTO' for Customer List
		var filter = "popup_password_changesSet(Userid='" + userId + "',Opassword='" + password + "',Password='" + newPassword + "',Cpassword='" +conPassword+ "',AppType='" +vApptype+ "')";
		filter = filter.replace(/&/g, "%26");
		filter = filter.replace(/#/g, "%23");
		filter = filter.replace(/%/g, "%25");
		var urlToCall = serviceUrl + filter;
		OData.request({ 
	...
	validateFieldsForPswdChng: function(){
		...
		if(ConfirmPswdFlag){
			if(vNewPswd.getValue().trim().length >= 6){//match(passw)){
	...
	changePassword: function(pwd){
		...
		//});*/
		 ...
		 oPassChangeDialog = new sap.ui.commons.Dialog({
		...
		var oChangeButton = new sap.ui.commons.Button({
						...
						{
						busyDialog.open();
						var newpwd = oNPassword.getValue().toUpperCase();
						var oldpwd = oPassword.getValue().toUpperCase();
						
						oCurrent.callBackend(userId, pswd, oNPassword.getValue(), oPassword.getValue());
						busyDialog.close();
						}
						else
						{
		...
		/*var oPassChangeLayout = new sap.ui.commons.layout.MatrixLayout(); Changed by Visithra.R on 03.08.2020
		...
		var oFlexboxPassChange = new sap.m.FlexBox({
		 items: [
		  // layoutTitle, * Changed by visithra on 03.04.2020
		   oFlexboxUsername,
		   oFlexboxPassword,
		   oFlexboxNPassword,
		   oFlexboxCPassword,
	...
	authenticateUser:function(){
		...
		var filter = "auth_userSet('"+userId+"|G|"+pswd+"')";
		...
		OData.request({ 
		    ...
		    function (data, response){
		    		...
		    		//busyDialog.close();
		    		if(data.Message.substr(0,7) == "SUCCESS"){
			    		sessionStorage.setItem("uName", userId);
			    		sessionStorage.setItem("uType", data.Message.substr(8,3));
			    	...
			    	else if(data.Message.substr(0,7) != "SUCCESS"){
			    		busyDialog.close();
			    		isAuthentic = false;
		    ...
		    function(err){
		    	 busyDialog.close();
		    	 sap.m.MessageBox.alert("Error while Login. Please try again");
	...
	getcust : function(){
		...
		var oMainSplitApp;
		if(urlSplitHome[2] == "192.168.203.108:8000"){
			 var serviceUrlCSO = "http://192.168.203.108:8000/sap/opu/odata/sap/ZUI5_CSO_SRV/";
			 var globaluser = "hlf_batch";
			 var globalpass = "hlf_batch";
		 }else if(urlSplitHome[2] == "192.168.203.109:8000"){
			 var serviceUrlCSO = "http://192.168.203.109:8000/sap/opu/odata/sap/ZUI5_CSO_SRV/";
			 var globaluser = "hlf_batch";
			 var globalpass = "hlf_batch";
		 }else if(urlSplitHome[2] == "hlfsapdev.halifa-bobo.com:8000"){
			 var serviceUrlCSO = "http://hlfsapdev.halifa-bobo.com:8000/sap/opu/odata/sap/ZUI5_CSO_SRV/";
			 var globaluser = "hlf_batch";
			 var globalpass = "hlf_batch";
		 }else{
			 var serviceUrlCSO = "http://" + urlSplitHome[2] + "/sap/opu/odata/sap/ZUI5_CSO_SRV/";
			 var globaluser = "hlf_batch";
			 var globalpass = "hlf_batch";
		 }
		
		var urlToSap = "cso_getcustSet(Customer='" + customer + "')";
					...
					for (var i = 0; i < data.results.length; i++) {
						...
						oArrayIntTransModelSingleSperson
								.push({
									...
									BEZEI : data.results[i].KUNNR
									
											+ "  "
											+ data.results[i].BEZEI
											+ "  "
											+ data.results[i].NAME1
											+ "  "
											+ data.results[i].NAME2
											+ "  "
											+ data.results[i].STRAS
											+ "  "
											+ data.results[i].PSTLZ
											+ "  "
											+ data.results[i].TELF1
											+ "  "
											+ data.results[i].ORT01
											+ "  "
											+ data.results[i].LAND1
								});
					}
